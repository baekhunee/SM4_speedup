...
b32 S(b32 b); // 查S盒
b32 Left(b32 a, short length); // 循环左移
b32 L1(b32 a); // 线性变换L1
b32 L2(b32 a); // 线性变换L2
...
void thread4(b32** X, b32* Key)
{
	thread* threads = new thread[thread_num];
	int i = 0;
	for (i = 0; i < thread_num; i++)
		threads[i] = thread(Thread4, ref(X), ref(Key), i);
	for (i = 0; i < thread_num; i++)
		threads[i].join();
}
...
void thread8(b32** X, b32* Key)
{
	thread* threads = new thread[2 * thread_num];
	int i = 0;
	for (i = 0; i < 2 * thread_num; i++)
		threads[i] = thread(Thread8, ref(X), ref(Key), i);
	for (i = 0; i < 2 * thread_num; i++)
		threads[i].join();
}
