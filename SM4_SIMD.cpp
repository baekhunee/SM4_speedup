...
#define L132(value, offset) ((value << offset) | value >> (32 - offset))
typedef b32* SM4_Key;
int key_initial(b8* key, SM4_Key* sm4_key)
{
	b32 K[4];
	b32 tmp32;
	b8* tmp8 = (b8*)&tmp32;
	*sm4_key = (b32*)malloc(32 * sizeof(b32));
	if (*sm4_key == (void*)0) {
		return 0;
	}
	...
	for (int i = 0; i < 32; i++) {
		tmp32 = K[1] ^ K[2] ^ K[3] ^ CK[i];
		for (int j = 0; j < 4; j++) {
			tmp8[j] = Sbox[tmp8[j]];
		}
		(*sm4_key)[i] = K[0] ^ tmp32 ^ L132(tmp32, 13) ^ L132(tmp32, 23);
		K[0] = K[1];
		K[1] = K[2];
		K[2] = K[3];
		K[3] = (*sm4_key)[i];
...
{
    __m256i X[4], tmp[4];
    __m256i mask = _mm256_set1_epi32(0xFF);//32bit分组数据装载（全部设成0xFF）
    //加载数据
    ...
    for (int i = 0; i < 32; i++) 
    {
        ...
        tmp[1] = _mm256_xor_si256(tmp[1], _mm256_i32gather_epi32((const int*)Sbox2,
            _mm256_and_si256(tmp[0], mask), 4));
        tmp[0] = _mm256_srli_epi32(tmp[0], 8);

        tmp[1] = _mm256_xor_si256(tmp[1], _mm256_i32gather_epi32((const int*)Sbox3,
            _mm256_and_si256(tmp[0], mask), 4));
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
    ...
    _mm256_storeu_si256((__m256i*)p + 3, PACK3_EPI32(X[3], X[2], X[1], X[0]));
}
void SM4_Enc(b8* m, b8* p, SM4_Key sm4_key)
{
    SM4_simd(m, p, sm4_key, 0);
}
void SM4_Dec(b8* m, b8* p, SM4_Key sm4_key)
{
    SM4_simd(m, p, sm4_key, 1);
}

int main()
{
    unsigned char key[16 * 8] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab,0xcd, 0xef,
                                 0xfe, 0xdc, 0xba, 0x98,0x76, 0x54, 0x32, 0x10 };
    unsigned char m[16 * 8] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
                                0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10 };
    SM4_Key Key;
    int tmp = key_initial(key, &Key);
    if (tmp)
    {
